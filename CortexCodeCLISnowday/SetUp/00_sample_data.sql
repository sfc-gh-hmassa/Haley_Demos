...
(
  ...
  $$SELECT
    ...
    ORDER BY INVOICE_COUNT DESC, APPROVAL_STATUS;$$,
  'COUNT(*)',
  'APPROVAL_STATUS',
  'Core'
),
...
(
  ...
  $$SELECT
      SUM(INVOICE_AMOUNT) AS TOTAL_SPEND
    FROM SILVER_AP_INVOICES
    WHERE VENDOR_NAME = 'Acme Industrial Supply';$$,
...
(
  ...
  $$SELECT
      ...
      SUM(INVOICE_AMOUNT) AS TOTAL_SPEND
    FROM SILVER_AP_INVOICES
    WHERE INVOICE_DATE >= '2025-01-01'::DATE
      AND INVOICE_DATE <  '2026-01-01'::DATE
    GROUP BY INVOICE_MONTH
    ORDER BY INVOICE_MONTH;$$,
  'SUM(INVOICE_AMOUNT)',
...
(
  ...
  $$WITH mx AS (
    ...
    SELECT s.*
    FROM SILVER_AP_INVOICES s
    JOIN mx
      ON s.INVOICE_DATE >= DATEADD('DAY', -30, mx.MAX_INVOICE_DATE)
