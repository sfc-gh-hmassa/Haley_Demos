...
def find_similar_products(session, description):
    """Find similar products using Cortex Search Service"""
    try:
        # Use the existing Cortex Search Service with available indexed columns
        # Build the JSON query string properly
        query_json = {
            "query": description,
            "columns": ["line_item_amount", "customer_id", "product_name_description", "invoice_number", "product_id", "rate", "quantity", "file_path"],
            "limit": 5
        }
...
def main():
    st.title("📄 PDF Visualizer & Data Extractor")
    ...
    uploaded_file = st.file_uploader(
        "Choose a PDF file", 
        type=['pdf'],
        help="Upload a PDF file to visualize and extract data from"
    )
    
    if uploaded_file is not None:
        with st.spinner("🔄 Uploading PDF to Snowflake stage..."):
        ...
        if final_filename and ai_extract_filename and pdf_content:
            ...
            with st.spinner("🤖 AI is analyzing the PDF and extracting line items..."):
    ...
    if st.session_state['upload_successful'] and st.session_state['pdf_content'] is not None:
    ...
    # Cortex Search Comparison Section
    if st.session_state['upload_successful'] and st.session_state['extracted_data'] is not None:
        st.markdown("---")
        st.subheader("🔍 Cortex Search Comparison & Negotiation Analysis")
        
        extracted_data = st.session_state['extracted_data']
        
        # Debug: Show the structure of extracted_data
        st.info(f"🔍 Extracted data type: {type(extracted_data)}")
        st.info(f"🔍 Extracted data content: {extracted_data}")
            ...
            for i, description in enumerate(response_data['description']):
                with st.expander(f"📦 {description[:50]}{'...' if len(description) > 50 else ''}"):
                    ...
                    with st.spinner(f"🔍 Searching for similar products..."):
                        similar_products = find_similar_products(session, description)
                            ...
                            for i, product in enumerate(similar_products):
                                ...
                                if file_path:
                                    ...
                                    with st.expander(f"📄 {filename} - Invoice #{invoice_num} - Customer: {customer_id}"):
                                        ...
                                        if st.button(f"🔗 Get Download Link", key=f"url_{filename}_{i}"):
